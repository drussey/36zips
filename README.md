...
| `data/ids.json` | Stable row ids, keyed `artist|song|quote[:40]|price_k`. `curate.py` issues the next number to a new row and never reuses a deleted row's number, so `#e<id>` links survive edits |
...
| `flags` | `implied_unit` (kilo not named in the line), `retrospective` (line refers to an earlier era), `outlier`, `hyperbole`, `homage`, `meta`, `derived`, `posthumous`, `profit_not_price`, `wordplay`, `companion` (second row split out of a line that prices two eras; on the chart but not the map, where its partner row stands for the song) |
