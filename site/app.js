(function () {
  ...
  (function globalBox() {
    ...
    for (const r of rows) {
      const y = by + r.dy;
      el('text', { class: 'gbox-label', x: bx + 12, y: y + 3.5, text: `${names[r.k]} (${r.list.length})` }, g);
      const x = bx + 12 + labelW;
      r.list.forEach((e, i) => {
        dot(g, e, x + (i % perRow) * step, y + Math.floor(i / perRow) * step);
      });
    }
  })();
  ...
  const nUS = us.length, nAbroad = D.entries.filter(e => e.region === 'global' && e.on_map).length, nUnk = D.entries.filter(e => e.on_map).length - nUS - nAbroad;
