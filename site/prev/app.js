(function () {
  ...
  function dot(parent, e, x, y) {
    const g = el('g', { class: 'dot', 'data-id': e.id, transform: `translate(${x},${y})` }, parent);
    el('circle', { class: 'ring', r: R }, g);
    el('circle', { class: 'core', r: R, fill: color(e.usd_k) }, g);
    el('circle', { class: 'spec', r: R, fill: 'url(#spec)' }, g);
    return g;
  }
  ...
  const TH = 320, TW = W, ML = 54, MR = 20, MT = 18, MB = 34;
  const tl = $('#timeline');
  ...
  for (let y = Y0; y <= Y1; y++) {
    ...
    if (major && y < Y1) el('text', { class: 'yr', x: xOf(y + 0.5), y: yOf(0) + 24, text: String(y).slice(2) }, tl);
  }
  const tDots = el('g', { class: 'dots' }, tl);
  const sortedT = D.entries.slice().sort((a, b) => a.usd_k - b.usd_k);
  for (const e of sortedT) {
    const j = ((e.id * 7919) % 13 - 6) * 0.35; // small fixed jitter so same-day, same-price bars don't stack exactly
    dot(tDots, e, xOf(frac(e)) + j, yOf(e.usd_k));
  }
  ...
  function render(e) {
    ...
    card.innerHTML = `
      ...
      <div class="card-top">
        <div class="price">${fmtK(e.price_k, e.currency)}<small> / KILO</small></div>
